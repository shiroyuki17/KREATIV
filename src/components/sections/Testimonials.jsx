    ...
    <section id="stories" className="relative py-12 md:py-24">
      <div className="mx-auto max-w-6xl px-6">
        <div className="flex flex-wrap items-end justify-between gap-6">
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-brand-soft">{t("tst.eyebrow")}</p>
            <h2 className="mt-3 font-display text-[clamp(1.9rem,3.6vw,2.8rem)] font-bold text-brand text-glow tracking-tight">
              <BlurText text={t("tst.title")} />
            </h2>
          </div>
          <button
            onClick={() => nav("reviews")}
          ...
          >
            {t("tst.readStories")} <ArrowRight className="h-4 w-4" />
          </button>
        </div>

        <div className="mt-10 grid gap-4 md:grid-cols-3">
          {(reviews || []).slice(0, 3).map((review) => (
            <SpotlightCard key={review.id}>
              <div className="flex h-full flex-col p-6">
                <div className="flex gap-0.5">
                  {Array.from({ length: 5 }).map((_, i) => (
                    <Star key={i} className={`h-4 w-4 ${i < Math.round(review.stars) ? "fill-amber-400 text-amber-400" : "text-white/15"}`} />
                  ))}
                </div>
                <p className="mt-4 flex-1 text-[14px] leading-relaxed text-white/75">“{review.comment}”</p>
                <div className="mt-5 flex items-center gap-3 border-t border-white/8 pt-4">
                  <Avatar name={review.reviewerName} seed={review.id} size="h-10 w-10 text-[26px]" />
                  <div className="min-w-0">
                    <p className="truncate text-[13.5px] font-semibold">{review.reviewerName}</p>
                    <p className="truncate text-[11.5px] text-white/40">
                      {review.authorSide === "client" ? t("tst.sideClient") : t("tst.sideFreelancer")}
