...
export default function LiveToasts() {
  ...
  return (
    <div className="pointer-events-none fixed bottom-24 right-6 z-[60] flex w-[min(330px,calc(100vw-3rem))] flex-col gap-2.5">
      {toasts.map((toast) => {
        const { Icon, cls, page } = META[toast.kind] || FALLBACK_META;
        return (
          <div
            key={toast.id}
          ...
          >
            ...
            <button onClick={() => { nav(page); dismiss(toast.id); }} className="min-w-0 flex-1 text-left">
              <p className="text-[13px] font-semibold">{toast.title}</p>
              <p className="mt-0.5 text-[11.5px] leading-snug text-white/50">{toast.body}</p>
            </button>
            <button
